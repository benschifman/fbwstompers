...
format. **`/booking/` is live on the current WordPress site**, so it must 301 to
`/contact/` at cutover or its ranking and any inbound links are lost.
...
know whether it's still needed, so it has no redirect yet. If it's dead, point
it at `/contact/`; if it's live, it needs rebuilding.

If you deploy somewhere without `_redirects` support (plain Apache, SiteGround),
these need translating into `.htaccess` rules instead.
